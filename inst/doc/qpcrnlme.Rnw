...
ctest <- qpcr_nlme(response="Fluorescence", cycle="Cycle", gene="Target", treatment="Timefactor", brep="Content", well="Well", data=pt6c, cutoff=100, nGQ=1, verbose=FALSE)
print(ctest)
@


$\Delta\Delta c(t)$ comparisons to the control time (0 hours) can be performed by the function ddctcomp. Besides the definition of a control gene and control treatment levels, following arguments can be modified:
\begin{itemize}
...
ddctcomp(ctest, control_treatment="0", control_gene="eEf", conf.level=0.95, ratio_ddct=TRUE, adjusted=TRUE)
@
